...
// in the environment.
func Get(key string, def ...interface{}) *dvar.Var {
...
func Set(key, value string) (err error) {
	...
	if err != nil {
		err = werr.WithMsgErrF(err, `set environment key-value failed with key "%s", value "%s"`, key, value)
	}
	return
}
...
func Remove(key ...string) (err error) {
	for _, v := range key {
		if err = os.Unsetenv(v); err != nil {
			err = werr.WithMsgErrF(err, `delete environment key failed with key "%s"`, v)
			return err
		}
	}
	return nil
