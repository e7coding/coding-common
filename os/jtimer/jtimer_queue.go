...
type priorityQueue struct {
	...
	nextPriority *jatomic.Int64     // nextPriority stores the next priority value of the heap, which is used to check if necessary to call the Pop of heap by Timer.
}
...
func newPriorityQueue() *priorityQueue {
	queue := &priorityQueue{
		...
		nextPriority: jatomic.NewInt64(math.MaxInt64),
	}
...
func (q *priorityQueue) Push(value interface{}, priority int64) {
	...
	nextPriority := q.nextPriority.Load()
	if priority >= nextPriority {
		return
	}
	q.nextPriority.Store(priority)
}
...
func (q *priorityQueue) Pop() interface{} {
	...
	if v := heap.Pop(q.heap); v != nil {
		...
		q.nextPriority.Store(nextPriority)
