...
import (
	"context"
	dredis2 "github.com/coding-common/dredis"
	"time"
...
func NewAdapterRedis(redis *dredis2.Redis) *AdapterRedis {
	return &AdapterRedis{
		redis: redis,
	}
}
...
func (c *AdapterRedis) Set(ctx context.Context, key interface{}, value interface{}, duration time.Duration) (err error) {
	...
	if value == nil || duration < 0 {
		...
		if duration == 0 {
			...
			_, err = c.redis.Set(ctx, redisKey, value, dredis2.SetOption{TTLOption: dredis2.TTLOption{PX: dconv.PtrInt64(duration.Milliseconds())}})
		}
	}
	return err
}
...
func (c *AdapterRedis) SetMap(ctx context.Context, data map[interface{}]interface{}, duration time.Duration) error {
	...
	if duration == 0 {
		err := c.redis.MSet(ctx, dconv.Map(data))
		if err != nil {
			return err
		}
	}
...
func (c *AdapterRedis) Get(ctx context.Context, key interface{}) (*dvar.Var, error) {
	return c.redis.Get(ctx, dconv.String(key))
}
...
func (c *AdapterRedis) GetOrSet(ctx context.Context, key interface{}, value interface{}, duration time.Duration) (result *dvar.Var, err error) {
	result, err = c.Get(ctx, key)
	if err != nil {
		return nil, err
	}
	if result.IsNil() {
		return dvar.New(value), c.Set(ctx, key, value, duration)
	}
	return
}
...
// It deletes the `key` if `duration` < 0 or given `value` is nil, but it does nothing
// if `value` is a function and the function result is nil.
func (c *AdapterRedis) GetOrSetFunc(ctx context.Context, key interface{}, f Func, duration time.Duration) (result *dvar.Var, err error) {
	v, err := c.Get(ctx, key)
	if err != nil {
		return nil, err
	}
...
// writing mutex lock for concurrent safety purpose.
func (c *AdapterRedis) GetOrSetFuncLock(ctx context.Context, key interface{}, f Func, duration time.Duration) (result *dvar.Var, err error) {
...
func (c *AdapterRedis) Contains(ctx context.Context, key interface{}) (bool, error) {
	n, err := c.redis.Exists(ctx, dconv.String(key))
	if err != nil {
		return false, err
	}
	return n > 0, nil
...
func (c *AdapterRedis) Keys(ctx context.Context) ([]interface{}, error) {
	...
	return dconv.Interfaces(keys), nil
}
...
func (c *AdapterRedis) Update(ctx context.Context, key interface{}, value interface{}) (oldValue *dvar.Var, exist bool, err error) {
	var (
		v        *dvar.Var
		oldPTTL  int64
		redisKey = dconv.String(key)
	...
	if oldPTTL == -1 {
		...
		_, err = c.redis.Set(ctx, redisKey, value, dredis2.SetOption{TTLOption: dredis2.TTLOption{PX: dconv.PtrInt64(oldPTTL)}})
	}
...
func (c *AdapterRedis) GetExpire(ctx context.Context, key interface{}) (time.Duration, error) {
	pttl, err := c.redis.PTTL(ctx, dconv.String(key))
	if err != nil {
		return 0, err
	}
	switch pttl {
...
func (c *AdapterRedis) Remove(ctx context.Context, keys ...interface{}) (lastValue *dvar.Var, err error) {
	if len(keys) == 0 {
		return nil, nil
	}
	// Retrieves the last key value.
	if lastValue, err = c.redis.Get(ctx, dconv.String(keys[len(keys)-1])); err != nil {
		return nil, err
	}
	// Deletes all given keys.
	_, err = c.redis.Del(ctx, dconv.Strings(keys)...)
	return
}
