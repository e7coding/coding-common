...
import (
	...
	"github.com/coding-common/container/list"
	"math"
	"time"
...
type AdapterMemory struct {
	...
	eventList   *list.SafeList     // eventList is the asynchronous event list for internal data synchronization.
	closed      *atomic.Bool       // closed controls the cache closed or not.
}
...
func doNewAdapterMemory() *AdapterMemory {
	...
	dtimer.AddSingleton(context.Background(), time.Second, c.syncEventAndClearExpired)
	return c
}
...
func (c *AdapterMemory) Get(ctx context.Context, key interface{}) (*dvar.Var, error) {
	item, ok := c.data.Get(key)
	if ok && !item.IsExpired() {
		c.handleLruKey(ctx, key)
		return dvar.New(item.v), nil
	}
	return nil, nil
}
...
func (c *AdapterMemory) GetOrSet(ctx context.Context, key interface{}, value interface{}, duration time.Duration) (*dvar.Var, error) {
	defer c.handleLruKey(ctx, key)
	v, err := c.Get(ctx, key)
	if err != nil {
		return nil, err
...
// It deletes the `key` if `duration` < 0 or given `value` is nil, but it does nothing
// if `value` is a function and the function result is nil.
func (c *AdapterMemory) GetOrSetFunc(ctx context.Context, key interface{}, f Func, duration time.Duration) (*dvar.Var, error) {
	defer c.handleLruKey(ctx, key)
	v, err := c.Get(ctx, key)
	if err != nil {
		return nil, err
...
// writing mutex lock for concurrent safety purpose.
func (c *AdapterMemory) GetOrSetFuncLock(ctx context.Context, key interface{}, f Func, duration time.Duration) (*dvar.Var, error) {
	defer c.handleLruKey(ctx, key)
	v, err := c.Get(ctx, key)
	if err != nil {
		return nil, err
...
func (c *AdapterMemory) GetExpire(ctx context.Context, key interface{}) (time.Duration, error) {
	if item, ok := c.data.Get(key); ok {
		c.handleLruKey(ctx, key)
		return time.Duration(item.e-dtime.TimestampMilli()) * time.Millisecond, nil
	}
	return -1, nil
}
...
func (c *AdapterMemory) Remove(ctx context.Context, keys ...interface{}) (*dvar.Var, error) {
	defer c.lru.Remove(keys...)
	return c.doRemove(ctx, keys...)
}

func (c *AdapterMemory) doRemove(_ context.Context, keys ...interface{}) (*dvar.Var, error) {
	...
	for _, key := range removedKeys {
		c.eventList.PushBack(&adapterMemoryEvent{
			k: key,
			e: dtime.TimestampMilli() - 1000,
		})
	}
	return dvar.New(value), nil
}
...
func (c *AdapterMemory) Update(ctx context.Context, key interface{}, value interface{}) (oldValue *dvar.Var, exist bool, err error) {
	v, exist, err := c.data.Update(key, value)
	if exist {
		c.handleLruKey(ctx, key)
	}
	return dvar.New(v), exist, err
}
...
func (c *AdapterMemory) doSetWithLockCheck(ctx context.Context, key interface{}, value interface{}, duration time.Duration) (result *dvar.Var, err error) {
...
func (c *AdapterMemory) syncEventAndClearExpired(ctx context.Context) {
	...
	var (
		expireSet  *gset.Set
		expireTime int64
		currentEk  = c.makeExpireKey(dtime.TimestampMilli())
	)
