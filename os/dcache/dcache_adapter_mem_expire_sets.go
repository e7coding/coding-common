...
type memoryExpireSets struct {
	...
	expireSets map[int64]*set.SafeSet
}

func newMemoryExpireSets() *memoryExpireSets {
	return &memoryExpireSets{
		expireSets: make(map[int64]*set.SafeSet),
	}
}

func (d *memoryExpireSets) Get(key int64) (result *set.SafeSet) {
	d.mu.RLock()
...
func (d *memoryExpireSets) GetOrNew(key int64) (result *set.SafeSet) {
