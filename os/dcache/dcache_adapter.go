...
type Adapter interface {
	...
	Get(ctx context.Context, key interface{}) (*dvar.Var, error)
	...
	GetOrSet(ctx context.Context, key interface{}, value interface{}, duration time.Duration) (result *dvar.Var, err error)
	...
	// It deletes the `key` if `duration` < 0 or given `value` is nil, but it does nothing
	// if `value` is a function and the function result is nil.
	GetOrSetFunc(ctx context.Context, key interface{}, f Func, duration time.Duration) (result *dvar.Var, err error)
	...
	// writing mutex lock for concurrent safety purpose.
	GetOrSetFuncLock(ctx context.Context, key interface{}, f Func, duration time.Duration) (result *dvar.Var, err error)
	...
	Update(ctx context.Context, key interface{}, value interface{}) (oldValue *dvar.Var, exist bool, err error)
	...
	Remove(ctx context.Context, keys ...interface{}) (lastValue *dvar.Var, err error)
