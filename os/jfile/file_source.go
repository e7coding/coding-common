...
func MainPkgPath() string {
	...
	for i := 1; i < 10000; i++ {
		if pc, file, _, ok := runtime.Caller(i); ok {
			...
			if fn := runtime.FuncForPC(pc); fn != nil {
				array := jstr.Split(fn.Name(), ".")
				if array[0] != "main" {
					continue
				}
			}
			if jregex.IsMatchString(`package\s+main\s+`, GetContents(file)) {
				mainPkgPath.Store(Dir(file))
				return Dir(file)
			}
		} else {
			break
	...
	if lastFile != "" {
		for path = Dir(lastFile); len(path) > 1 && Exists(path) && path[len(path)-1] != os.PathSeparator; {
			...
			for _, v := range files {
				if jregex.IsMatchString(`package\s+main\s+`, GetContents(v)) {
					mainPkgPath.Store(path)
					return path
				}
			}
			path = Dir(path)
