...
import (
	...
	"github.com/coding-common/internal/utils"
	"github.com/coding-common/text/dregex"
)
...
func StrToTime(str string, format ...string) (*Time, error) {
	...
	if match = timeRegex1.FindStringSubmatch(str); len(match) > 0 && match[1] != "" {
		...
		return NewFromTime(time.Date(0, time.Month(1), 1, hour, mins, sec, nsec, local)), nil
	} else {
		return nil, werr.WithMsgF(`unsupported time converting for string "%s"`, str)
	}
	...
	if match[6] != "" {
		...
		if len(zone) <= 6 {
			...
			if h > 24 || m > 59 || s > 59 {
				return nil, werr.WithMsgF(`invalid zone string "%s"`, match[6])
			}
			operation := match[5]
	...
	if month <= 0 || day <= 0 {
		return nil, werr.WithMsgF(`invalid time string "%s"`, str)
	}
	return NewFromTime(time.Date(year, time.Month(month), day, hour, mins, sec, nsec, local)), nil
}
...
func ConvertZone(strTime string, toZone string, fromZone ...string) (*Time, error) {
	...
	if len(fromZone) > 0 {
		if l, err = time.LoadLocation(fromZone[0]); err != nil {
			err = werr.WithMsgErrF(err, `time.LoadLocation failed for name "%s"`, fromZone[0])
			return nil, err
		} else {
	...
	if l, err = time.LoadLocation(toZone); err != nil {
		err = werr.WithMsgErrF(err, `time.LoadLocation failed for name "%s"`, toZone)
		return nil, err
	} else {
...
func ParseTimeFromContent(content string, format ...string) *Time {
	...
	if len(format) > 0 {
		for _, item := range format {
			match, err = dregex.MatchString(formatToRegexPattern(item), content)
			if err != nil {
...
func ParseDuration(s string) (duration time.Duration, err error) {
	...
	if utils.IsNumeric(s) {
		...
		if err != nil {
			err = werr.WithMsgErrF(err, `strconv.ParseInt failed for string "%s"`, s)
			return 0, err
		}
	...
	if len(match) == 3 {
		...
		if err != nil {
			err = werr.WithMsgErrF(err, `strconv.ParseInt failed for string "%s"`, match[1])
			return 0, err
		}
		s = fmt.Sprintf(`%dh%s`, num*24, match[2])
		duration, err = time.ParseDuration(s)
		if err != nil {
			err = werr.WithMsgErrF(err, `time.ParseDuration failed for string "%s"`, s)
		}
		return
	}
	duration, err = time.ParseDuration(s)
