...
func homeUnix() (string, error) {
	...
	if err := cmd.Run(); err != nil {
		err = werr.WithMsgErr(err, `retrieve home directory failed`)
		return "", err
	}

	result := strings.TrimSpace(stdout.String())
	if result == "" {
		return "", werr.WithMsg("blank output when reading home directory")
	}
...
func homeWindows() (string, error) {
	...
	if home == "" {
		return "", werr.WithMsg("environment keys HOMEDRIVE, HOMEPATH and USERPROFILE are empty")
	}
