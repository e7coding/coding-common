...
func doScanDir(depth int, path string, pattern string, recursive bool, handler func(path string) string) ([]string, error) {
	if depth >= maxScanDepth {
		return nil, werr.WithMsgF("directory scanning exceeds max recursive depth: %d", maxScanDepth)
	}
	var (
		list      []string
		file, err = Open(path)
	...
	if err != nil {
		err = werr.WithMsgErrF(err, `read directory files failed from path "%s"`, path)
		return nil, err
	}
	var (
		filePath string
