...
func Copy(src string, dst string, option ...CopyOption) error {
	if src == "" {
		return werr.WithMsg("source path cannot be empty")
	}
	if dst == "" {
		return werr.WithMsg("destination path cannot be empty")
	}
	srcStat, srcStatErr := os.Stat(src)
	if srcStatErr != nil {
		if os.IsNotExist(srcStatErr) {
			return werr.WithMsgErrF(
				srcStatErr,
				`the src path "%s" does not exist`,
				src,
			)
		}
		return werr.WithMsgErrF(srcStatErr, `call os.Stat on "%s" failed`, src)
	}
	dstStat, dstStatErr := os.Stat(dst)
	if dstStatErr != nil && !os.IsNotExist(dstStatErr) {
		return werr.WithMsgErrF(dstStatErr, `call os.Stat on "%s" failed`, dst)
	}
...
func CopyFile(src, dst string, option ...CopyOption) (err error) {
	...
	if src == "" {
		return werr.WithMsg("source file cannot be empty")
	}
	if dst == "" {
		return werr.WithMsg("destination file cannot be empty")
	}
	...
	defer func() {
		if e := inFile.Close(); e != nil {
			err = werr.WithMsgErrF(e, `file close failed for "%s"`, src)
		}
	}()
	var outFile *os.File
	...
	defer func() {
		if e := outFile.Close(); e != nil {
			err = werr.WithMsgErrF(e, `file close failed for "%s"`, dst)
		}
	}()
	if _, err = io.Copy(outFile, inFile); err != nil {
		err = werr.WithMsgErrF(err, `io.Copy failed from "%s" to "%s"`, src, dst)
		return
	}
	if usedOption.Sync {
		if err = outFile.Sync(); err != nil {
			err = werr.WithMsgErrF(err, `file sync failed for file "%s"`, dst)
			return
		}
	}
...
func CopyDir(src string, dst string, option ...CopyOption) (err error) {
	var usedOption = getCopyOption(option...)
	if src == "" {
		return werr.WithMsg("source directory cannot be empty")
	}
	if dst == "" {
		return werr.WithMsg("destination directory cannot be empty")
	}
	// If src and dst are the same path, it does nothing.
	if src == dst {
		return nil
	...
	if !si.IsDir() {
		return werr.WithMsg("source is not a directory")
	}
	if usedOption.PreserveMode {
	...
	if err != nil {
		err = werr.WithMsgErrF(err, `read directory failed for path "%s"`, src)
		return
	}
