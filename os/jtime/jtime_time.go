...
// Time is a wrapper for jtime.Time for additional features.
type Time struct {
	wrapper
}

// iUnixNano is an interface definition commonly for custom jtime.Time wrapper.
type iUnixNano interface {
	UnixNano() int64
}

// New creates and returns a Time object with given parameter.
// The optional parameter is the time object which can be type of: jtime.Time/*jtime.Time, string or integer.
// Example:
// New("2024-10-29")
// New(1390876568)
// New(t) // The t is type of jtime.Time.
...
// NewFromTime creates and returns a Time object with given jtime.Time object.
...
func (t *Time) AddStr(duration string) (*Time, error) {
	if d, err := time.ParseDuration(duration); err != nil {
		err = jerr.WithMsgErrF(err, `time.ParseDuration failed for string "%s"`, duration)
		return nil, err
	} else {
		return t.Add(d), nil
	}
...
// Note that it overwrites the same implementer of `jtime.Time`.
func (t *Time) UnmarshalText(data []byte) error {
	...
	return jerr.WithMsgF(`invalid time value: %s`, data)
}
