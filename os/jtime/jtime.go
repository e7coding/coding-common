...
func StrToTime(str string, format ...string) (*Time, error) {
	...
	if match = timeRegex1.FindStringSubmatch(str); len(match) > 0 && match[1] != "" {
		...
		return nil, jerr.WithMsgF(`unsupported time converting for string "%s"`, str)
	}
	...
	if match[6] != "" {
		...
		if len(zone) <= 6 {
			...
			if h > 24 || m > 59 || s > 59 {
				return nil, jerr.WithMsgF(`invalid zone string "%s"`, match[6])
			}
			operation := match[5]
	...
	if month <= 0 || day <= 0 {
		return nil, jerr.WithMsgF(`invalid time string "%s"`, str)
	}
...
func ConvertZone(strTime string, toZone string, fromZone ...string) (*Time, error) {
	...
	if len(fromZone) > 0 {
		if l, err = time.LoadLocation(fromZone[0]); err != nil {
			err = jerr.WithMsgErrF(err, `time.LoadLocation failed for name "%s"`, fromZone[0])
			return nil, err
		} else {
	...
	if l, err = time.LoadLocation(toZone); err != nil {
		err = jerr.WithMsgErrF(err, `time.LoadLocation failed for name "%s"`, toZone)
		return nil, err
	} else {
...
//
// Very note that it supports unit "d" more than function jtime.ParseDuration.
func ParseDuration(s string) (duration time.Duration, err error) {
	var num int64
	if utils.IsNumeric(s) {
		num, err = strconv.ParseInt(s, 10, 64)
		if err != nil {
			err = jerr.WithMsgErrF(err, `strconv.ParseInt failed for string "%s"`, s)
			return 0, err
		}
		return time.Duration(num), nil
	}
	match, err := jregex.MatchString(`^([\-\d]+)[dD](.*)$`, s)
	if err != nil {
		return 0, err
	}
	if len(match) == 3 {
		...
		if err != nil {
			err = jerr.WithMsgErrF(err, `strconv.ParseInt failed for string "%s"`, match[1])
			return 0, err
		}
		s = fmt.Sprintf(`%dh%s`, num*24, match[2])
		duration, err = time.ParseDuration(s)
		if err != nil {
			err = jerr.WithMsgErrF(err, `time.ParseDuration failed for string "%s"`, s)
		}
		return
	}
	duration, err = time.ParseDuration(s)
