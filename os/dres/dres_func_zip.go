...
import (
	...
	"github.com/coding-common/internal/fileinfo"
...
func doZipPathWriter(srcPath string, zipWriter *zip.Writer, option ...Option) error {
	...
	absolutePath, err = dfile.Search(srcPath)
	if err != nil {
		return err
	}
	if dfile.IsDir(absolutePath) {
		files, err = dfile.ScanDir(absolutePath, "*", true)
		if err != nil {
			return err
		}
	} else {
	...
	if headerPrefix != "" && dfile.IsDir(absolutePath) {
		headerPrefix += "/"
	}
	...
	if headerPrefix == "" {
		if usedOption.KeepPath {
			...
			headerPrefix = dfile.Basename(absolutePath)
		}
	}
	...
	for _, file := range files {
		...
		if subFilePath != "" {
			subFilePath = dfile.Dir(subFilePath)
		}
	...
	if headerPrefix != "" {
		...
		for {
			name = strings.ReplaceAll(dfile.Basename(tmpPath), `\`, `/`)
...
func zipFile(path string, prefix string, zw *zip.Writer) error {
	...
	if err != nil {
		err = werr.WithMsgErrF(err, `os.Open failed for path "%s"`, path)
		return nil
	}
	...
	info, err := file.Stat()
	if err != nil {
		err = werr.WithMsgErrF(err, `read file stat failed for path "%s"`, path)
		return err
	}
	...
	if err != nil {
		err = werr.WithMsgErrF(err, `create zip header failed for %#v`, header)
		return err
	}
	if !info.IsDir() {
		if _, err = io.Copy(writer, file); err != nil {
			err = werr.WithMsgErrF(err, `io.Copy failed for file "%s"`, path)
			return err
		}
	}
	return nil
...
func createFileHeader(info os.FileInfo, prefix string) (*zip.FileHeader, error) {
	...
	if err != nil {
		err = werr.WithMsgErrF(err, `create file header failed for name "%s"`, info.Name())
		return nil, err
	}
	if len(prefix) > 0 {
		header.Name = prefix + `/` + header.Name
		header.Name = strings.ReplaceAll(header.Name, `\`, `/`)
		header.Name, _ = dregex.ReplaceString(`/{2,}`, `/`, header.Name)
	}
	return header, nil
}
