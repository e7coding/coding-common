...
import (
	...
	"github.com/coding-common/encoding/dbase64"
	"github.com/coding-common/encoding/dcompress"
...
import "github.com/coding-common/os/gres"
...
func PackWithOption(srcPaths string, option Option) ([]byte, error) {
	...
	return dcompress.Gzip(buffer.Bytes(), 9)
}
...
func PackToFile(srcPaths, dstPath string, keyPrefix ...string) error {
	...
	return dfile.PutBytes(dstPath, data)
}
...
func PackToGoFile(srcPath, goFilePath, pkgName string, keyPrefix ...string) error {
	...
	return dfile.PutContents(
		goFilePath,
		fmt.Sprintf(dstr.TrimLeft(packedGoSourceTemplate), pkgName, dbase64.EncodeToStr(data)),
	)
}
...
func Unpack(path string) ([]*File, error) {
	realPath, err := dfile.Search(path)
	if err != nil {
		return nil, err
	}
	return UnpackContent(dfile.GetContents(realPath))
}
...
func UnpackContent(content string) ([]*File, error) {
	...
	if isHexStr(content) {
		...
		data, err = dcompress.UnGzip(hexStrToBytes(content))
		if err != nil {
			return nil, err
		}
	} else if isBase64(content) {
		// New version packing string using base64.
		b, err := dbase64.DecodeStr(content)
		if err != nil {
			return nil, err
		}
		data, err = dcompress.UnGzip(b)
		if err != nil {
			return nil, err
		}
	} else {
		data, err = dcompress.UnGzip([]byte(content))
		if err != nil {
			return nil, err
		}
	}
	...
	if err != nil {
		err = werr.WithMsgErrF(err, `create zip reader failed`)
		return nil, err
	}
