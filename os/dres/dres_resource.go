...
func New() *Resource {
	return &Resource{
		tree: tree.NewBTree(defaultTreeM, func(v1, v2 interface{}) int {
...
func (r *Resource) Load(path string, prefix ...string) error {
	...
	return r.Add(dfile.GetContents(realPath), prefix...)
}
...
func (r *Resource) doScanDir(path string, pattern string, recursive bool, onlyFile bool) []*File {
	...
	r.tree.ForeachFrom(path, true, func(key, value interface{}) bool {
		...
		for _, p := range patterns {
			if match, err := filepath.Match(p, dfile.Basename(name)); err == nil && match {
...
func (r *Resource) Export(src, dst string, option ...ExportOption) error {
	...
	for _, file := range files {
		...
		if exportOption.RemovePrefix != "" {
			name = dstr.TrimLeftStr(name, exportOption.RemovePrefix)
		}
		name = dstr.Trim(name, `\/`)
		if name == "" {
			continue
		}
		path = dfile.Join(dst, name)
		if file.FileInfo().IsDir() {
			err = dfile.Mkdir(path)
		} else {
			err = dfile.PutBytes(path, file.Content())
		}
		if err != nil {
			return err
		}
...
func (r *Resource) Dump() {
	...
	r.tree.Foreach(func(key, value interface{}) bool {
		...
		fmt.Printf(
			"%v %8s %s\n",
			dtime.New(info.ModTime()).ISO8601(),
			dfile.FormatSize(info.Size()),
			key,
		)
		return true
	})
