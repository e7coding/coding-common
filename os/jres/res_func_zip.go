...
import (
	...
	"github.com/e7coding/coding-common/internal/fileinfo"
...
func doZipPathWriter(srcPath string, zipWriter *zip.Writer, option ...Option) error {
	...
	absolutePath, err = jfile.Search(srcPath)
	if err != nil {
		return err
	}
	if jfile.IsDir(absolutePath) {
		files, err = jfile.ScanDir(absolutePath, "*", true)
		if err != nil {
			return err
		}
	} else {
	...
	if headerPrefix != "" && jfile.IsDir(absolutePath) {
		headerPrefix += "/"
	}
	...
	if headerPrefix == "" {
		if usedOption.KeepPath {
			...
			headerPrefix = jfile.Basename(absolutePath)
		}
	}
	...
	for _, file := range files {
		...
		if subFilePath != "" {
			subFilePath = jfile.Dir(subFilePath)
		}
	...
	if headerPrefix != "" {
		...
		for {
			name = strings.ReplaceAll(jfile.Basename(tmpPath), `\`, `/`)
...
func zipFile(path string, prefix string, zw *zip.Writer) error {
	...
	if err != nil {
		err = jerr.WithMsgErrF(err, `os.Open failed for path "%s"`, path)
		return nil
	}
	...
	info, err := file.Stat()
	if err != nil {
		err = jerr.WithMsgErrF(err, `read file stat failed for path "%s"`, path)
		return err
	}
	...
	if err != nil {
		err = jerr.WithMsgErrF(err, `create zip header failed for %#v`, header)
		return err
	}
	if !info.IsDir() {
		if _, err = io.Copy(writer, file); err != nil {
			err = jerr.WithMsgErrF(err, `io.Copy failed for file "%s"`, path)
			return err
		}
	}
	return nil
...
func createFileHeader(info os.FileInfo, prefix string) (*zip.FileHeader, error) {
	...
	if err != nil {
		err = jerr.WithMsgErrF(err, `create file header failed for name "%s"`, info.Name())
		return nil, err
	}
	if len(prefix) > 0 {
		header.Name = prefix + `/` + header.Name
		header.Name = strings.ReplaceAll(header.Name, `\`, `/`)
		header.Name, _ = jregex.ReplaceString(`/{2,}`, `/`, header.Name)
	}
	return header, nil
}
