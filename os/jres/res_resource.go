...
func New() *Resource {
	return &Resource{
		tree: jtree.NewBTree(defaultTreeM, func(v1, v2 interface{}) int {
...
func (r *Resource) Load(path string, prefix ...string) error {
	...
	return r.Add(jfile.GetContents(realPath), prefix...)
}
...
func (r *Resource) doScanDir(path string, pattern string, recursive bool, onlyFile bool) []*File {
	...
	first := true
	r.tree.ForeachFrom(path, true, func(key, value interface{}) bool {
		if first {
		...
		for _, p := range patterns {
			if match, err := filepath.Match(p, jfile.Basename(name)); err == nil && match {
...
func (r *Resource) Export(src, dst string, option ...ExportOption) error {
	...
	for _, file := range files {
		...
		if exportOption.RemovePrefix != "" {
			name = jstr.TrimLeftStr(name, exportOption.RemovePrefix)
		}
		name = jstr.Trim(name, `\/`)
		if name == "" {
			continue
		}
		path = jfile.Join(dst, name)
		if file.FileInfo().IsDir() {
			err = jfile.Mkdir(path)
		} else {
			err = jfile.PutBytes(path, file.Content())
		}
		if err != nil {
			return err
		}
...
func (r *Resource) Dump() {
	...
	r.tree.Foreach(func(key, value interface{}) bool {
		info = value.(*File).FileInfo()
		fmt.Printf(
			"%v %8s %s\n",
			jtime.New(info.ModTime()).ISO8601(),
			jfile.FormatSize(info.Size()),
			key,
		)
		return true
	})
