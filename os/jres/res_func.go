...
import (
	...
	"github.com/e7coding/coding-common/encoding/jbase64"
...
import "github.com/e7coding/coding-common/os/gres"
...
func PackWithOption(srcPaths string, option Option) ([]byte, error) {
	...
	return jcompress.Gzip(buffer.Bytes(), 9)
}
...
func PackToFile(srcPaths, dstPath string, keyPrefix ...string) error {
	...
	return jfile.PutBytes(dstPath, data)
}
...
func PackToGoFile(srcPath, goFilePath, pkgName string, keyPrefix ...string) error {
	...
	return jfile.PutContents(
		goFilePath,
		fmt.Sprintf(jstr.TrimLeft(packedGoSourceTemplate), pkgName, jbase64.EncodeToStr(data)),
	)
}
...
func Unpack(path string) ([]*File, error) {
	realPath, err := jfile.Search(path)
	if err != nil {
		return nil, err
	}
	return UnpackContent(jfile.GetContents(realPath))
}
...
func UnpackContent(content string) ([]*File, error) {
	...
	if isHexStr(content) {
		...
		data, err = jcompress.UnGzip(hexStrToBytes(content))
		if err != nil {
			return nil, err
		}
	} else if isBase64(content) {
		// New version packing string using base64.
		b, err := jbase64.DecodeStr(content)
		if err != nil {
			return nil, err
		}
		data, err = jcompress.UnGzip(b)
		if err != nil {
			return nil, err
		}
	} else {
		data, err = jcompress.UnGzip([]byte(content))
		if err != nil {
			return nil, err
		}
	}
	...
	if err != nil {
		err = jerr.WithMsgErrF(err, `create zip reader failed`)
		return nil, err
	}
