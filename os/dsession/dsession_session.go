...
type Session struct {
	...
	data    *wmap.SafeStrAnyMap // Current Session data, which is retrieved from Storage.
...
func (s *Session) SetId(id string) error {
	if s.start {
		return werr.WithMsg("session already started")
	}
	s.id = id
	return nil
}
...
func (s *Session) Get(key string, def ...interface{}) (value *dvar.Var, err error) {
	if s.id == "" {
		return nil, nil
	}
	if err = s.init(); err != nil {
	...
	if v != nil {
		return dvar.New(v), nil
	}
	if v = s.data.Get(key); v != nil {
		return dvar.New(v), nil
	}
	if len(def) > 0 {
		return dvar.New(def[0]), nil
	}
	return nil, nil
}
