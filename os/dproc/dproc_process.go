...
func (p *Process) Start(ctx context.Context) (int, error) {
	...
	ctx, span = tr.Start(
		otel.GetTextMapPropagator().Extract(
			ctx,
			propagation.MapCarrier(denv.Map()),
		),
		dstr.Join(os.Args, " "),
...
func (p *Process) Send(data []byte) error {
	...
	return werr.WithMsg("invalid process")
}
...
func (p *Process) Kill() (err error) {
	err = p.Process.Kill()
	if err != nil {
		err = werr.WithMsgErrF(err, `kill process failed for pid "%d"`, p.Process.Pid)
		return err
	}
	if p.Manager != nil {
