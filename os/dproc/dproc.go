...
import (
	...
	"github.com/coding-common/os/denv"
...
const (
	...
	tracingInstrumentName = "github.com/coding-common/os/gproc.Process"
)
...
func SetPPid(ppid int) error {
	if ppid > 0 {
		return os.Setenv(envKeyPPid, dconv.String(ppid))
	} else {
...
func SearchBinaryPath(file string) string {
	...
	switch runtime.GOOS {
	case "windows":
		envPath := denv.Get("PATH", denv.Get("Path")).String()
		if dstr.Contains(envPath, ";") {
			array = dstr.SplitAndTrim(envPath, ";")
		} else if dstr.Contains(envPath, ":") {
			array = dstr.SplitAndTrim(envPath, ":")
		}
		if dfile.Ext(file) != ".exe" {
			file += ".exe"
		}

	default:
		array = dstr.SplitAndTrim(denv.Get("PATH").String(), ":")
	}
	if len(array) > 0 {
		path := ""
		for _, v := range array {
			path = v + dfile.Separator + file
			if dfile.Exists(path) && dfile.IsFile(path) {
				return path
			}
		}
	}
