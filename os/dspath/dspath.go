...
type SPath struct {
	paths *arr.SafeStrArr     // The searching directories array.
	cache *wmap.SafeStrStrMap // Searching cache map, it is not enabled if it's nil.
}
...
func (sp *SPath) Set(path string) (realPath string, err error) {
	...
	if realPath == "" {
		...
		if realPath == "" {
			realPath = dfile.RealPath(dfile.Pwd() + dfile.Separator + path)
		}
	}
	if realPath == "" {
		return realPath, werr.WithMsgF(`path "%s" does not exist`, path)
	}
	// The set path must be a directory.
	if dfile.IsDir(realPath) {
		realPath = strings.TrimRight(realPath, dfile.Separator)
		...
		return "", werr.WithMsg(path + " should be a folder")
	}
}
	...
	if sp.cache == nil {
		...
		if len(indexFiles) > 0 && isDir {
			...
			for _, file := range indexFiles {
				path = filePath + dfile.Separator + file
