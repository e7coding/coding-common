...
func (sp *SPath) nameFromPath(filePath, rootPath string) string {
	name := dstr.Replace(filePath, rootPath, "")
...
func (sp *SPath) addToCache(filePath, rootPath string) {
	...
	if idDir {
		if files, err := dfile.ScanDir(filePath, "*", true); err == nil {
			// fmt.Println("gspath add to cache:", filePath, files)
			for _, path := range files {
				sp.cache.SetIfNotExist(sp.nameFromPath(path, rootPath), sp.makeCacheValue(path, dfile.IsDir(path)))
			}
		}
	}
}
