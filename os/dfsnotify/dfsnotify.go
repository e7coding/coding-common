...
type Watcher struct {
	...
	cache     *dcache.Cache       // Used for repeated event filter.
	nameSet   *set.SafeStrSet     // Used for AddOnce feature.
	callbacks *wmap.SafeStrAnyMap // Path(file/folder) to callbacks mapping.
...
var (
	...
	callbackIdMap       = wmap.NewSafeIntAnyMap() // Global callback id to callback function mapping.
	callbackIdGenerator = atomic.NewInt()         // Atomic id generator for callback.
)
...
func New() (*Watcher, error) {
	w := &Watcher{
		cache:     dcache.New(),
		events:    queue.New(),
		nameSet:   set.NewSafeStrSet(),
		closeChan: make(chan struct{}),
		callbacks: wmap.NewSafeStrAnyMap(),
	}
...
func RemoveCallback(callbackId int) error {
	...
	if callback == nil {
		return werr.WithMsgF(`callback for id %d not found`, callbackId)
	}
