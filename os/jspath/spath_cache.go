...
func (sp *SPath) nameFromPath(filePath, rootPath string) string {
	name := jstr.Replace(filePath, rootPath, "")
...
func (sp *SPath) addToCache(filePath, rootPath string) {
	...
	if idDir {
		if files, err := jfile.ScanDir(filePath, "*", true); err == nil {
			// fmt.Println("gspath add to cache:", filePath, files)
			for _, path := range files {
				sp.cache.SetIfNotExist(sp.nameFromPath(path, rootPath), sp.makeCacheValue(path, jfile.IsDir(path)))
			}
		}
	}
}
...
func (sp *SPath) addMonitorByPath(path string) {
	...
	_, _ = jfsnotify.Add(path, func(event *jfsnotify.Event) {
		...
		switch {
		case event.IsRemove():
			sp.cache.Delete(sp.nameFromPath(event.Path, path))

		case event.IsRename():
			if !jfile.Exists(event.Path) {
				sp.cache.Delete(sp.nameFromPath(event.Path, path))
			}
