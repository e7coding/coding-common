...
import (
	...
	"github.com/coding-common/net/dipv4"
	"github.com/coding-common/net/dtrace/internal/provider"
...
var (
	intranetIps, _           = dipv4.GetIntranetIpArray()
...
func GetBaggageMap(ctx context.Context) *wmap.StrAnyMap {
	return NewBaggage(ctx).GetMap()
}

// GetBaggageVar retrieves value and returns a *dvar.Var for specified key from baggage.
func GetBaggageVar(ctx context.Context, key string) *dvar.Var {
...
func WithUUID(ctx context.Context, uuid string) (context.Context, error) {
	return WithTraceID(ctx, dstr.Replace(uuid, "-", ""))
}
