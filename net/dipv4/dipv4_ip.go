...
func GetIpArray() (ips []string, err error) {
	...
	if err != nil {
		err = werr.WithMsgErr(err, `net.InterfaceAddrs failed`)
		return nil, err
	}
...
func GetIntranetIp() (ip string, err error) {
	...
	if len(ips) == 0 {
		return "", werr.WithMsg("no intranet ip found")
	}
	return ips[0], nil
}
...
func GetIntranetIpArray() (ips []string, err error) {
	...
	if err != nil {
		err = werr.WithMsgErr(err, `net.Interfaces failed`)
		return ips, err
	}
	for _, interFace := range interFaces {
		...
		if err != nil {
			err = werr.WithMsgErr(err, `interFace.Addrs failed`)
			return ips, err
		}
