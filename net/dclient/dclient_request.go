...
import (
	...
	"github.com/coding-common/encoding/djson"

	"github.com/coding-common/internal/httputil"
...
func (c *Client) prepareRequest(ctx context.Context, method, url string, data ...interface{}) (req *http.Request, err error) {
	...
	if len(c.prefix) > 0 {
		url = c.prefix + dstr.Trim(url)
	}
	if !dstr.ContainsI(url, httpProtocolName) {
	...
	if len(data) > 0 {
		...
		case httpHeaderContentTypeXml:
			...
			case string, []byte:
				params = dconv.String(data[0])
			default:
				if b, err := djson.New(data[0]).ToXml(); err != nil {
					return nil, err
				} else {
					params = string(b)
				}
	...
	if method == http.MethodGet {
		...
		if req, err = http.NewRequest(method, url, bodyBuffer); err != nil {
			err = werr.WithMsgErrF(err, `http.NewRequest failed with method "%s" and URL "%s"`, method, url)
			return nil, err
		}
	} else {
		if allowFileUploading && strings.Contains(params, httpParamFileHolder) {
			...
			for _, item := range strings.Split(params, "&") {
				...
				if len(array[1]) > 6 && strings.Compare(array[1][0:6], httpParamFileHolder) == 0 {
					...
					if !dfile.Exists(path) {
						return nil, werr.WithMsgF(`"%s" does not exist`, path)
					}
					var (
						file          io.Writer
						formFileName  = dfile.Basename(path)
					...
					var f *os.File
					if f, err = dfile.Open(path); err != nil {
						return nil, err
					}
					...
					if err = f.Close(); err != nil {
						return nil, werr.WithMsgErrF(err, `close file descriptor failed for "%s"`, path)
					}
			...
			if err = writer.Close(); err != nil {
				return nil, werr.WithMsgErrF(err, `form writer close failed`)
			}
			...
			if req, err = http.NewRequest(method, url, bytes.NewReader(paramBytes)); err != nil {
				err = werr.WithMsgErrF(err, `http.NewRequest failed for method "%s" and URL "%s"`, method, url)
				return nil, err
			}
			if v, ok := c.header[httpHeaderContentType]; ok {
				...
				} else if dregex.IsMatchString(httpRegexParamJson, params) {
...
func (c *Client) callRequest(req *http.Request) (resp *Response, err error) {
	...
	for {
		...
		if resp.Response, err = c.Do(req); err != nil {
			err = werr.WithMsgErrF(err, `request failed`)
