...
func LoadKeyCrt(crtFile, keyFile string) (*tls.Config, error) {
	crtPath, err := dfile.Search(crtFile)
	if err != nil {
		return nil, err
	}
	keyPath, err := dfile.Search(keyFile)
	if err != nil {
		return nil, err
	}
	crt, err := tls.LoadX509KeyPair(crtPath, keyPath)
	if err != nil {
		err = werr.WithMsgErrF(err, `tls.LoadX509KeyPair failed for certFile "%s", keyFile "%s"`, crtPath, keyPath)
		return nil, err
	}
