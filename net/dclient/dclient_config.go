...
func (c *Client) SetHeaderRaw(headers string) *Client {
	for _, line := range dstr.SplitAndTrim(headers, "\n") {
		array, _ := dregex.MatchString(httpRegexHeaderRaw, line)
		if len(array) >= 3 {
...
func (c *Client) SetTLSKeyCrt(crtFile, keyFile string) error {
	...
	if err != nil {
		return werr.WithMsgErr(err, "LoadKeyCrt failed")
	}
	if v, ok := c.Transport.(*http.Transport); ok {
	...
	return werr.WithMsg(`cannot set TLSClientConfig for custom Transport of the client`)
}
