...
var (
	...
	serverMapping = wmap.NewSafeStrAnyMap()
)
...
func (s *Server) Close() (err error) {
	...
	err = s.conn.Close()
	if err != nil {
		err = werr.WithMsgErr(err, "connection failed")
	}
	return
}
...
func (s *Server) Run() error {
	...
	if err != nil {
		err = werr.WithMsgErrF(err, `net.ResolveUDPAddr failed for address "%s"`, s.address)
		return err
	}
	listenedConn, err := net.ListenUDP("udp", addr)
	if err != nil {
		err = werr.WithMsgErrF(err, `net.ListenUDP failed for address "%s"`, s.address)
		return err
	}
	s.mu.Lock()
