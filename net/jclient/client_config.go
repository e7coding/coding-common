...
func (c *Client) SetHeaderRaw(headers string) *Client {
	for _, line := range jstr.SplitAndTrim(headers, "\n") {
		array, _ := jregex.MatchString(httpRegexHeaderRaw, line)
		if len(array) >= 3 {
...
func (c *Client) SetTLSKeyCrt(crtFile, keyFile string) error {
	...
	if err != nil {
		return jerr.WithMsgErr(err, "LoadKeyCrt failed")
	}
	if v, ok := c.Transport.(*http.Transport); ok {
	...
	return jerr.WithMsg(`cannot set TLSClientConfig for custom Transport of the client`)
}
