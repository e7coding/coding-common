...
var serverMapping = wmap.NewSafeStrAnyMap()
...
func GetServer(name ...interface{}) *Server {
	...
	return serverMapping.GetOrPutFunc(serverName, func() interface{} {
...
func (s *Server) Run() (err error) {
	if s.handler == nil {
		err = werr.WithMsg("start running failed: socket handler not defined")
		return
	}
	if s.tlsConfig != nil {
		// TLS Server
		...
		s.mu.Unlock()
		if err != nil {
			err = werr.WithMsgErrF(err, `tls.Listen failed for address "%s"`, s.address)
			return
		}
	} else {
		// Normal Server
		...
		if tcpAddr, err = net.ResolveTCPAddr("tcp", s.address); err != nil {
			err = werr.WithMsgErrF(err, `net.ResolveTCPAddr failed for address "%s"`, s.address)
			return err
		}
		s.mu.Lock()
		...
		s.mu.Unlock()
		if err != nil {
			err = werr.WithMsgErrF(err, `net.ListenTCP failed for address "%s"`, s.address)
			return err
		}
	}
	// Listening loop.
	for {
		...
		if conn, err = s.listen.Accept(); err != nil {
			err = werr.WithMsgErrF(err, `Listener.Accept failed`)
			return err
		} else if conn != nil {
...
func (s *Server) GetListenedAddress() string {
	if !dstr.Contains(s.address, FreePortAddress) {
		return s.address
	}
	var (
		address      = s.address
	...
	address = dstr.Replace(address, FreePortAddress, fmt.Sprintf(`:%d`, listenedPort))
	return address
}
