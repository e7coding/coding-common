	...
	for {
		if _, err := c.Write(data); err != nil {
			...
			if len(retry) == 0 || retry[0].Count == 0 {
				err = werr.WithMsgErr(err, `Write data failed`)
				return err
			}
			if len(retry) > 0 {
				retry[0].Count--
	...
	for {
		if length < 0 && index > 0 {
			...
			if err = c.SetReadDeadline(time.Now().Add(c.bufferWaitRecv)); err != nil {
				err = werr.WithMsgErr(err, `SetReadDeadline for connection failed`)
				return nil, err
			}
		}
...
func (c *Conn) SetDeadline(t time.Time) (err error) {
	...
	if err != nil {
		err = werr.WithMsgErrF(err, `SetDeadline for connection failed with "%s"`, t)
	}
	return err
}
...
func (c *Conn) SetDeadlineRecv(t time.Time) (err error) {
	...
	if err != nil {
		err = werr.WithMsgErrF(err, `SetDeadlineRecv for connection failed with "%s"`, t)
	}
	return err
}
...
func (c *Conn) SetDeadlineSend(t time.Time) (err error) {
	...
	if err != nil {
		err = werr.WithMsgErrF(err, `SetDeadlineSend for connection failed with "%s"`, t)
	}
	return err
}
