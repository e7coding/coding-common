...
import (
	...
	"github.com/coding-common/encoding/durl"
...
func BuildParams(params interface{}, noUrlEncode ...bool) (encodedParamStr string) {
	...
	m, urlEncode := dconv.Map(params, dconv.MapOption{
		OmitEmpty: true,
	}), true
	if len(m) == 0 {
		return dconv.String(params)
	...
	if urlEncode {
		for k, v := range m {
			if dstr.Contains(k, fileUploadingKey) || dstr.Contains(dconv.String(v), fileUploadingKey) {
				urlEncode = false
				break
			}
		}
