...
import "github.com/e7coding/coding-common/os/jfile"
...
func LoadPath(path string, options Options) (*Json, error) {
	if p, err := jfile.Search(path); err != nil {
		return nil, err
	} else {
		path = p
	}
	if options.Type == "" {
		options.Type = ContentType(jfile.Ext(path))
	}
	return loadContentWithOptions(jfile.GetBytesWithCache(path), options)
}
