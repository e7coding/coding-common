...
import (
	...
	"github.com/coding-common/internal/reflection"
	"github.com/coding-common/internal/rwmutex"
...
func (j *Json) setValue(pattern string, value interface{}, removed bool) error {
	...
	for i := 0; i < length; i++ {
		...
		case []interface{}:
			...
			if err != nil {
				err = werr.WithMsgErrF(err, `strconv.Atoi 失败："%s"`, array[i])
				return err
			}
...
func (j *Json) convertValue(value interface{}) (convertedValue interface{}, err error) {
	...
	default:
		...
		case reflect.Array:
			return dconv.Interfaces(value), nil

		case reflect.Slice:
			return dconv.Interfaces(value), nil

		case reflect.Map:
			return dconv.Map(value), nil
		...
		case reflect.Struct:
			...
			if utils.IsNil(convertedValue) {
				convertedValue = dconv.Map(value)
			}
			if utils.IsNil(convertedValue) {
				err = werr.WithMsgF(`不支持的类型 "%s"`, reflect.TypeOf(value))
			}
			return
