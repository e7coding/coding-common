...
import "github.com/coding-common/os/dfile"
...
func LoadPath(path string, options Options) (*Json, error) {
	if p, err := dfile.Search(path); err != nil {
		return nil, err
	} else {
		path = p
	}
	if options.Type == "" {
		options.Type = ContentType(dfile.Ext(path))
	}
	return loadContentWithOptions(dfile.GetBytesWithCache(path), options)
}
