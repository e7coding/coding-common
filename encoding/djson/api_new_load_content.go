...
func loadContentWithOptions(data []byte, options Options) (*Json, error) {
	...
	options.Type = ContentType(dstr.TrimLeft(
		string(options.Type), "."),
	)
	switch options.Type {
	...
	case ContentTypeXml:
		data, err = dxml.ToJson(data)

	case ContentTypeYaml, ContentTypeYml:
		data, err = dyaml.ToJson(data)

	case ContentTypeToml:
		data, err = dtoml.ToJson(data)

	case ContentTypeIni:
		data, err = dini.ToJSON(data)

	case ContentTypeProperties:
		data, err = dproperties.ToJson(data)
	...
	switch result.(type) {
	case string, []byte:
		return nil, werr.WithMsgF(`json decoding failed for content: %s`, data)
	}
...
func isXmlContent(data []byte) bool {
	return dregex.IsMatch(`^\s*<.+>[\S\s]+<.+>\s*$`, data)
}

func isYamlContent(data []byte) bool {
	return !dregex.IsMatch(`[\n\r]*[\s\t\w\-\."]+\s*=\s*"""[\s\S]+"""`, data) &&
		!dregex.IsMatch(`[\n\r]*[\s\t\w\-\."]+\s*=\s*'''[\s\S]+'''`, data) &&
		((dregex.IsMatch(`^[\n\r]*[\w\-\s\t]+\s*:\s*".+"`, data) ||
			dregex.IsMatch(`^[\n\r]*[\w\-\s\t]+\s*:\s*\w+`, data)) ||
			(dregex.IsMatch(`[\n\r]+[\w\-\s\t]+\s*:\s*".+"`, data) ||
