...
func Valid(data interface{}) bool {
	return json.Valid(dconv.Bytes(data))
}
...
func Decode(data interface{}, options ...Options) (interface{}, error) {
	var value interface{}
	if err := DecodeTo(dconv.Bytes(data), &value, options...); err != nil {
		return nil, err
	} else {
		return value, nil
	}
...
func DecodeTo(data interface{}, v interface{}, options ...Options) (err error) {
	decoder := json.NewDecoder(bytes.NewReader(dconv.Bytes(data)))
	if len(options) > 0 {
	...
	if err = decoder.Decode(v); err != nil {
		err = werr.WithMsgErr(err, `json Decode failed`)
	}
	return
}
...
func DecodeToJson(data interface{}, options ...Options) (*Json, error) {
	if v, err := Decode(dconv.Bytes(data), options...); err != nil {
		return nil, err
	} else {
		if len(options) > 0 {
