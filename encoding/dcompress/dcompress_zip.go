...
func ZipPath(fileOrFolderPaths, dstFilePath string, prefix ...string) error {
	...
	if err != nil {
		return werr.WithMsgErrF(err, `os.Create 失败，文件名="%s"`, dstFilePath)
	}
	...
	for _, path := range strings.Split(fileOrFolderPaths, ",") {
		...
		if err = doZipPathWriter(path, dfile.RealPath(dstFilePath), zw, prefix...); err != nil {
			return err
		}
	}
	return nil
...
func doZipPathWriter(fileOrFolderPath, exclude string, zw *zip.Writer, prefix ...string) error {
	fileOrFolderPath, err := dfile.Search(fileOrFolderPath)
	if err != nil {
		return err
	}

	var files []string
	if dfile.IsDir(fileOrFolderPath) {
		files, err = dfile.ScanDir(fileOrFolderPath, "*", true)
		if err != nil {
			return err
		}
	} else {
	...
	if dfile.IsDir(fileOrFolderPath) {
		if headerPrefix != "" {
			...
			headerPrefix = dfile.Basename(fileOrFolderPath)
		}
	}
	...
	for _, file := range files {
		...
		relDir := dfile.Dir(file[len(fileOrFolderPath):])
		if relDir == "." {
			relDir = ""
		}
...
func UnZipFile(zippedFilePath, dstFolderPath string, zippedPrefix ...string) error {
	...
	if err != nil {
		return werr.WithMsgErrF(err, `zip.OpenReader 失败，文件名="%s"`, zippedFilePath)
	}
...
func UnZipContent(zippedContent []byte, dstFolderPath string, zippedPrefix ...string) error {
	...
	if err != nil {
		return werr.WithMsgErr(err, `zip.NewReader 失败`)
	}
...
func unZipFileWithReader(reader *zip.Reader, dstFolderPath string, zippedPrefix ...string) error {
	...
	if len(zippedPrefix) > 0 {
		prefix = dstr.Replace(zippedPrefix[0], `\`, `/`)
	}
	...
	for _, file := range reader.File {
		name := strings.Trim(dstr.Replace(file.Name, `\`, `/`), "/")
		if prefix != "" {
		...
		if file.FileInfo().IsDir() {
			if err := os.MkdirAll(dstPath, file.Mode()); err != nil {
				return werr.WithMsgErrF(err, `创建目录失败 "%s"`, dstPath)
			}
			continue
		}
		if err := os.MkdirAll(filepath.Dir(dstPath), 0755); err != nil {
			return werr.WithMsgErrF(err, `创建目录失败 "%s"`, filepath.Dir(dstPath))
		}
		fr, err := file.Open()
		if err != nil {
			return werr.WithMsgErr(err, `file.Open 失败`)
		}
...
func doCopyForUnZipFileWithReader(file *zip.File, fr io.ReadCloser, dstPath string) error {
	...
	if err != nil {
		return werr.WithMsgErrF(err, `os.OpenFile 失败，文件名="%s"`, dstPath)
	}
	...
	if _, err = io.Copy(fw, fr); err != nil {
		return werr.WithMsgErrF(err, `io.Copy 失败 从 "%s" 到 "%s"`, file.Name, dstPath)
	}
	return nil
}
...
func zipFile(filePath, prefix string, zw *zip.Writer) error {
	...
	if err != nil {
		return werr.WithMsgErrF(err, `os.Open 失败，文件名="%s"`, filePath)
	}
	...
	info, err := f.Stat()
	if err != nil {
		return werr.WithMsgErrF(err, `file.Stat 失败，文件名="%s"`, filePath)
	}
	...
	if err != nil {
		return werr.WithMsgErrF(err, `CreateHeader 失败，header="%#v"`, header)
	}
	if !info.IsDir() {
		if _, err = io.Copy(writer, f); err != nil {
			return werr.WithMsgErrF(err, `io.Copy 失败 从 "%s" 到 "%s"`, filePath, header.Name)
		}
	}
	return nil
}
...
func createFileHeader(info os.FileInfo, prefix string) (*zip.FileHeader, error) {
	...
	if err != nil {
		err = werr.WithMsgErrF(err, `zip.FileInfoHeader failed for info "%#v"`, info)
		return nil, err
	}
