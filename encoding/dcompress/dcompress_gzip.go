...
func Gzip(data []byte, level ...int) ([]byte, error) {
	...
	if len(level) > 0 {
		...
		if err != nil {
			return nil, werr.WithMsgErrF(err, `gzip.NewWriterLevel 失败，level="%d"`, level[0])
		}
	} else {
		writer = gzip.NewWriter(&buf)
	}
	if _, err = writer.Write(data); err != nil {
		return nil, werr.WithMsgErr(err, `writer.Write 失败`)
	}
	if err = writer.Close(); err != nil {
		return nil, werr.WithMsgErr(err, `writer.Close 失败`)
	}
...
func GzipFile(srcFilePath, dstFilePath string, level ...int) error {
	dstFile, err := dfile.Create(dstFilePath)
	if err != nil {
		return err
	}
...
func GzipPathWriter(filePath string, writer io.Writer, level ...int) error {
	...
	srcFile, err := dfile.Open(filePath)
	if err != nil {
		return err
	}
	...
	if len(level) > 0 {
		...
		if err != nil {
			return werr.WithMsgErr(err, `gzip.NewWriterLevel 失败`)
		}
	} else {
	...
	if _, err = io.Copy(gzipWriter, srcFile); err != nil {
		return werr.WithMsgErr(err, `io.Copy 失败`)
	}
	return nil
}
...
func UnGzip(data []byte) ([]byte, error) {
	...
	if err != nil {
		return nil, werr.WithMsgErr(err, `gzip.NewReader 失败`)
	}
	...
	if _, err = io.Copy(&buf, reader); err != nil {
		return nil, werr.WithMsgErr(err, `io.Copy 失败`)
	}
	return buf.Bytes(), nil
}
...
func UnGzipFile(srcFilePath, dstFilePath string) error {
	srcFile, err := dfile.Open(srcFilePath)
	if err != nil {
		return err
	}
	defer func(srcFile *os.File) {
	...
	if err != nil {
		return werr.WithMsgErr(err, `gzip.NewReader 失败`)
	}
