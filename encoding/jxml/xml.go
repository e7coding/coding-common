...
import (
	"github.com/e7coding/coding-common/encoding/jcharset"
...
func Decode(content []byte) (map[string]interface{}, error) {
	...
	if err != nil {
		err = jerr.WithMsgErr(err, `mxj.NewMapXml failed`)
	}
	return m, err
}
...
func DecodeWithoutRoot(content []byte) (map[string]interface{}, error) {
	...
	m, err := mxj.NewMapXml(res)
	if err != nil {
		err = jerr.WithMsgErrF(err, `mxj.NewMapXml failed`)
		return nil, err
	}
	for _, v := range m {
...
func Encode(m map[string]interface{}, rootTag ...string) ([]byte, error) {
	...
	if err != nil {
		err = jerr.WithMsgErrF(err, `mxj.Map.Xml failed`)
	}
	return b, err
}
...
func EncodeWithIndent(m map[string]interface{}, rootTag ...string) ([]byte, error) {
	...
	if err != nil {
		err = jerr.WithMsgErrF(err, `mxj.Map.XmlIndent failed`)
	}
	return b, err
}
...
func convert(xml []byte) (res []byte, err error) {
	var (
		...
		matchStr, _ = jregex.MatchString(patten, string(xml))
		xmlEncode   = "UTF-8"
	)
	if len(matchStr) == 2 {
	...
	res, err = jregex.Replace(patten, []byte(""), xml)
	if err != nil {
		return nil, err
	}
	if xmlEncode != "UTF-8" && xmlEncode != "UTF8" {
		dst, err := jcharset.Convert("UTF-8", xmlEncode, string(res))
		if err != nil {
			return nil, err
		}
		res = []byte(dst)
