...
func Decode(data []byte) (res map[string]interface{}, err error) {
	...
	if err != nil || pr == nil {
		err = werr.WithMsgErrF(err, `Lib magiconair load Properties data failed.`)
		return nil, err
	}
...
func Encode(data map[string]interface{}) (res []byte, err error) {
	...
	for _, key := range keys {
		_, _, err := pr.Set(key, dconv.String(flattened[key]))
		if err != nil {
			err = werr.WithMsgErrF(err, `Sets the property key to the corresponding value failed.`)
			return nil, err
		}
	}
	...
	if err != nil {
		err = werr.WithMsgErrF(err, `Properties Write buf failed.`)
		return nil, err
	}
