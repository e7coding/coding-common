...
import (
	...
	"github.com/coding-common/util/dconv/internal/localinterface"
)
...
// GTime converts `any` to *dtime.Time.
...
// If no `format` given, it converts `any` using dtime.NewFromTimeStamp if `any` is numeric,
// or using dtime.StrToTime if `any` is string.
func (c *Converter) GTime(any interface{}, format ...string) (*dtime.Time, error) {
	if empty.IsNil(any) {
		return nil, nil
	}
	...
	if len(s) == 0 {
		return dtime.New(), nil
	}
	// Priority conversion using given format.
	if len(format) > 0 {
		for _, item := range format {
			t, err := dtime.StrToTimeFormat(s, item)
			if err != nil {
				return nil, err
			}
			if t != nil {
	...
	if utils.IsNumeric(s) {
		...
		return dtime.NewFromTimeStamp(i), nil
	} else {
		return dtime.StrToTime(s)
	}
}
