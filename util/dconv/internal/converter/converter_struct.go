...
import (
	...
	"github.com/coding-common/util/dconv/internal/structcache"
)
...
func (c *Converter) Struct(params, pointer any, option ...StructOption) (err error) {
	...
	if pointer == nil {
		return werr.WithMsgF("object pointer cannot be nil")
	}
	...
	defer func() {
		if exception := recover(); exception != nil {
			if err = c.bindVarToReflectValue(fieldValue, srcValue, option); err != nil {
				err = werr.WithMsgErrF(err, `error binding srcValue to attribute "%s"`, cachedFieldInfo.FieldName())
			}
		}
	}()
	// Directly converting.
