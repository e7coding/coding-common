...
func doScanList(
	...
	if bindToAttrName == "" {
		return werr.WithMsgF(`bindToAttrName should not be empty`)
	}
	...
	for i := 0; i < arrayValue.Len(); i++ {
		...
		if len(relationDataMap) > 0 && !relationFromAttrValue.IsValid() {
			return werr.WithMsgF(`invalid relation fields specified: "%v"`, relationFields)
		}
		...
		default:
			return werr.WithMsgF(`unsupported attribute type: %s`, bindToAttrKind.String())
		}
	}
