...
func (sia *SafeIntArr) AppendBefore(index int, vals ...int) error {
	...
	if index < 0 || index >= len(sia.arr) {
		return werr.WithCodeF(code.NewErrCode(code.ParamErr), "index %d out of arr range %d", index, len(sia.arr))
	}
...
func (sia *SafeIntArr) Rand() (val int) {
	...
	val = sia.arr[drand.Intn(len(sia.arr))]
...
func (sia *SafeIntArr) Rands(size int) []int {
	...
	arr := make([]int, size)
	for i := 0; i < size; i++ {
		arr[i] = sia.arr[drand.Intn(len(sia.arr))]
	}
...
func (sia *SafeIntArr) UnmarshalValue(val interface{}) error {
	...
	switch val.(type) {
	case string, []byte:
		return json.UnmarshalUseNumber(dconv.Bytes(val), &sia.arr)
	default:
		sia.arr = dconv.SliceInt(val)
	}
