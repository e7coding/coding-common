...
func (aa *IntArr) Set(index int, val int) error {
	...
	if index < 0 || index >= len(aa.arr) {
		return werr.WithCodeF(code.NewErrCode(code.ParamErr), "index %d out of arr range %d", index, len(aa.arr))
	}
	aa.arr[index] = val
...
func (aa *IntArr) Rand() (val int) {
	...
	val = aa.arr[drand.Intn(len(aa.arr))]
...
func (aa *IntArr) Rands(size int) []int {
	...
	arr := make([]int, size)
	for i := 0; i < size; i++ {
		arr[i] = aa.arr[drand.Intn(len(aa.arr))]
	}
...
func (aa *IntArr) UnmarshalValue(val interface{}) error {
	...
	switch val.(type) {
	case string, []byte:
		return json.UnmarshalUseNumber(dconv.Bytes(val), &aa.arr)
	default:
		aa.arr = dconv.SliceInt(val)
	}
	return nil
}
