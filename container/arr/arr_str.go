...
func (sa *StrArr) SetAt(index int, value string) error {
	if index < 0 || index >= len(sa.array) {
		return werr.WithMsgF("index %d out of array range %d", index, len(sa.array))
	}
...
func (sa *StrArr) PopRandom() (string, bool) {
	...
	return sa.doRemove(drand.Intn(len(sa.array)))
}
...
func (sa *StrArr) PopRandoms(size int) []string {
	...
	for i := 0; i < size; i++ {
		out[i], _ = sa.doRemove(drand.Intn(len(sa.array)))
	}
	return out
}
...
func (sa *StrArr) Merge(src interface{}) *StrArr {
	return sa.Append(dconv.Strings(src)...)
}

// Fill 从指定索引开始，用相同值填充若干元素
func (sa *StrArr) Fill(start, num int, val string) error {
	if start < 0 || start > len(sa.array) {
		return werr.WithMsgF("index %d out of array range %d", start, len(sa.array))
	}
...
func (sa *StrArr) Random() (string, bool) {
	...
	return sa.array[drand.Intn(len(sa.array))], true
}
...
func (sa *StrArr) Randoms(size int) []string {
	...
	out := make([]string, size)
	for i := 0; i < size; i++ {
		out[i] = sa.array[drand.Intn(len(sa.array))]
	}
	return out
}
...
func (sa *StrArr) Shuffle() *StrArr {
	for i, j := range drand.Perm(len(sa.array)) {
...
func (sa *StrArr) UnmarshalValue(val interface{}) error {
	switch val.(type) {
	case string, []byte:
		return json.UnmarshalUseNumber(dconv.Bytes(val), &sa.array)
	default:
		sa.array = dconv.SliceStr(val)
		return nil
	}
}
