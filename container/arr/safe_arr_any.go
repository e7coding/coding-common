...
func (saa *SafeAnyArr) AppendBefore(index int, vals ...interface{}) error {
	...
	if index < 0 || index >= len(saa.arr) {
		return werr.WithCodeF(code.NewErrCode(code.ParamErr), "index %d out of arr range %d", index, len(saa.arr))
	}
...
func (saa *SafeAnyArr) UnmarshalValue(val interface{}) error {
	...
	switch val.(type) {
	case string, []byte:
		return json.UnmarshalUseNumber(dconv.Bytes(val), &saa.arr)
	default:
		saa.arr = dconv.SliceAny(val)
	}
	return nil
}
