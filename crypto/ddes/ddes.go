...
func EncECB(plainText []byte, key []byte, pad int) ([]byte, error) {
	...
	if err != nil {
		err = werr.WithMsgErrF(err, `des.NewCipher 失败，key="%s"`, key)
		return nil, err
	}
...
func EncECBTriple(plainText []byte, key []byte, pad int) ([]byte, error) {
	if len(key) != 16 && len(key) != 24 {
		return nil, werr.WithMsg("key 长度错误")
	}
	...
	if err != nil {
		err = werr.WithMsgErrF(err, `des.NewTripleDESCipher 失败，key="%s"`, newKey)
		return nil, err
	}
...
func EncCBC(plainText []byte, key []byte, iv []byte, pad int) ([]byte, error) {
	...
	if len(iv) != block.BlockSize() {
		return nil, werr.WithMsg("无效的 iv 长度")
	}
...
func Pad(text []byte, pad int) ([]byte, error) {
	...
	case NoPad:
		if len(text)%8 != 0 {
			return nil, werr.WithMsg("明文长度无效")
		}
	case PKCS5PAD:
		return PadPKCS5(text, 8), nil
	default:
		return nil, werr.WithMsgF("不支持的填充类型 %d", pad)
	}
	return text, nil
}
...
func UnPad(text []byte, pad int) ([]byte, error) {
	...
	case NoPad:
		if len(text)%8 != 0 {
			return nil, werr.WithMsg("密文长度无效")
		}
	case PKCS5PAD:
