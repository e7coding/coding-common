...
func EncCBC(plainText []byte, key []byte, iv ...[]byte) ([]byte, error) {
	...
	if err != nil {
		err = werr.WithMsgErrF(err, `aes.NewCipher 失败，key="%s"`, key)
		return nil, err
	}
...
func DecCBC(cipherText []byte, key []byte, iv ...[]byte) ([]byte, error) {
	...
	if len(cipherText) < blockSize {
		return nil, werr.WithMsg("cipherText 太短")
	}
	var ivValue []byte
	if len(iv) > 0 {
		ivValue = iv[0]
	...
	if len(cipherText)%blockSize != 0 {
		return nil, werr.WithMsg("cipherText 长度不是块大小的整数倍")
	}
...
func PKCS7UnPad(src []byte, blockSize int) ([]byte, error) {
	length := len(src)
	if blockSize <= 0 {
		return nil, werr.WithMsg(fmt.Sprintf("无效块大小：%d", blockSize))
	}
	if length == 0 || length%blockSize != 0 {
		return nil, werr.WithMsg("无效数据长度")
	}
	unpad := int(src[length-1])
	if unpad == 0 || unpad > blockSize {
		return nil, werr.WithMsg("无效填充大小")
	}
	for _, v := range src[length-unpad:] {
		if int(v) != unpad {
			return nil, werr.WithMsg("填充不匹配")
		}
	}
...
func EncGCM(plainText, key []byte, nonceOpt ...[]byte) ([]byte, error) {
	...
	block, err := aes.NewCipher(key)
	if err != nil {
		return nil, werr.WithMsgErrF(err, `aes.NewCipher 失败，key="%x"`, key)
	}
	// 2. 创建 GCM AEAD
	aead, err := cipher.NewGCM(block)
	if err != nil {
		return nil, werr.WithMsgErrF(err, `cipher.NewGCM 失败`)
	}
	// 3. 准备 nonce
	var nonce []byte
	if len(nonceOpt) > 0 {
		...
		if len(nonce) != aead.NonceSize() {
			return nil, werr.WithMsg(`提供的 nonce 长度不符 GCM.NonceSize()`)
		}
	} else {
		nonce = make([]byte, aead.NonceSize())
		if _, err = io.ReadFull(rand.Reader, nonce); err != nil {
			return nil, werr.WithMsgErrF(err, `生成随机 nonce 失败`)
		}
	}
...
func DecCFB(cipherText []byte, key []byte, unPad int, iv ...[]byte) ([]byte, error) {
	...
	// 密文长度检查
	if len(cipherText) < blockSize {
		return nil, werr.WithMsg("cipherText 长度不足")
	}
	// 选择 IV 并校验长度
	var ivValue []byte
	if len(iv) > 0 {
		...
		if len(ivValue) != blockSize {
			return nil, werr.WithMsg(fmt.Sprintf("IV 长度必须为 %d 字节，当前 %d 字节", blockSize, len(ivValue)))
		}
	} else {
		ivValue = []byte(IVDefaultValue)
	}
