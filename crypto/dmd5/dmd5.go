...
func Enc(data interface{}) (encrypt string, err error) {
	return EncBytes(dconv.Bytes(data))
}
...
func EncBytes(data []byte) (encrypt string, err error) {
	...
	if _, err = h.Write(data); err != nil {
		err = werr.WithMsgErr(err, "hash.Write 失败")
		return "", err
	}
...
func EncFile(path string) (encrypt string, err error) {
	...
	if err != nil {
		err = werr.WithMsgErrF(err, `os.Open 失败，path="%s"`, path)
		return "", err
	}

	h := md5.New()
	if _, err = io.Copy(h, f); err != nil {
		err = werr.WithMsgErr(err, "io.Copy 失败")
		return "", err
	}
