...
func doScanList(
	...
	if bindToAttrName == "" {
		return jerr.WithMsgF(`bindToAttrName should not be empty`)
	}
	...
	for i := 0; i < arrayValue.Len(); i++ {
		...
		if len(relationDataMap) > 0 && !relationFromAttrValue.IsValid() {
			return jerr.WithMsgF(`invalid relation fields specified: "%v"`, relationFields)
		}
		...
		default:
			return jerr.WithMsgF(`unsupported attribute type: %s`, bindToAttrKind.String())
		}
	}
