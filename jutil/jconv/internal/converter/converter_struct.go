...
import (
	...
	"github.com/e7coding/coding-common/jutil/jconv/internal/structcache"
)
...
func (c *Converter) Struct(params, pointer any, option ...StructOption) (err error) {
	...
	if pointer == nil {
		return jerr.WithMsgF("object pointer cannot be nil")
	}
	...
	defer func() {
		if exception := recover(); exception != nil {
			if err = c.bindVarToReflectValue(fieldValue, srcValue, option); err != nil {
				err = jerr.WithMsgErrF(err, `error binding srcValue to attribute "%s"`, cachedFieldInfo.FieldName())
			}
		}
	}()
	// Directly converting.
