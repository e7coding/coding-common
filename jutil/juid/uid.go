...
import (
	...
	"github.com/e7coding/coding-common/encoding/jhash"
...
var (
	sequence     jatomic.Uint32 // Sequence for unique purpose of the current process.
...
func init() {
	...
	if len(macs) > 0 {
		...
		s := strconv.FormatUint(uint64(jhash.SDBM32(macAddrBytes)), 36)
		copy(b, s)
		macAddrStr = string(b)
	}
...
func getDataHashStr(data []byte) []byte {
	b := []byte{'0', '0', '0', '0', '0', '0', '0'}
	s := strconv.FormatUint(uint64(jhash.SDBM32(data)), 36)
	copy(b, s)
	return b
}
